...
export function parseFrontmatter(
  ...
  if (!fm) return null;
  const type = typeof fm.type === "string" ? fm.type : fm.type == null ? "single" : "";
  ...
  if (type === "single") {
    ...
    return {
      ...
      // Keep the old truthiness: any non-string truthy value still counts as done.
      completed: typeof fm.completed === "string" ? fm.completed : fm.completed ? "true" : fm.completed === false ? false : null,
    };
  }
  if (type === "recurring") {
    const dows = Array.isArray(fm.daysOfWeek) ? fm.daysOfWeek : [];
    const daysOfWeek = dows
      .map((d: unknown) => WEEKDAY_LETTERS.indexOf(String(d).trim().toUpperCase()))
      .filter((n: number) => n >= 0);
