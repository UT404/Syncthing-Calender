...
export class CalendarUI {
  ...
  private segBtns: Record<CalView, HTMLElement> = {} as Record<CalView, HTMLElement>;
  ...
  private buildChrome() {
    ...
    this.calPanel.setCssStyles({ display: "none" });
  }
  ...
  private endPage() {
    ...
    window.setTimeout(() => {
      ...
      this.body.setCssStyles({ transform: "", transition: "" });
  ...
  private renderBack() {
    ...
    const v = this.state.view;
    this.backBtn.empty();
    if (v === "year" || v === "reminders") { this.backBtn.setCssStyles({ visibility: "hidden" }); return; }
    this.backBtn.setCssStyles({ visibility: "" });
  ...
  private renderMonth() {
    ...
    for (let i = 0; i < rows * 7; i++) {
      ...
      if (style === "details") {
        ...
        more.setCssStyles({ display: "none" });
      }
  ...
  private fitChips(grid: HTMLElement) {
    grid.querySelectorAll<HTMLElement>(".orc-cell").forEach((cell) => {
      ...
      chips.forEach((c) => c.setCssStyles({ display: "" }));
      more.setCssStyles({ display: "none" });
      ...
      for (let i = 0; i < chips.length; i++) {
        ...
        if (need > avail && !(remainingAfter === 0 && used + h <= avail)) {
          ...
          for (let j = i; j < chips.length; j++) chips[j].setCssStyles({ display: "none" });
          break;
        }
        used += h;
      }
  ...
  private makeGhost(col: HTMLElement, color: string, title: string): HTMLElement {
    ...
    g.setCssStyles({ left: "1px", width: "calc(100% - 3px)" });
