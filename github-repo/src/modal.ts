...
export class CalendarModal extends Modal {
  ...
  private get ops() {
    ...
    return this.kind === "list"
      ? { noun: "list", items: "reminder", root: s.remindersFolder(), all: s.getLists(), create: s.createList, rename: s.renameList, color: s.setListColor, del: s.deleteList, contents: s.listContents }
      : { noun: "calendar", items: "event", root: s.rootFolder(), all: s.getCalendars(), create: s.createCalendar, rename: s.renameCalendar, color: s.setCalendarColor, del: s.deleteCalendar, contents: s.folderContents };
  }
  onOpen() {
    const { contentEl, modalEl } = this;
    modalEl.addClass("orc-modal");
...
export class ConflictsModal extends Modal {
  constructor(app: App, private files: { path: string; basename: string }[], private onPick: (f: any) => void) { super(app); }
  onOpen() {
    const { contentEl, modalEl } = this;
    modalEl.addClass("orc-modal");
    contentEl.empty();
...
export class ReminderDetailsModal extends Modal {
  ...
  onOpen() {
    ...
    check.addEventListener("click", async () => { this.close(); try { await this.store.toggleReminder(r); } catch { new Notice("Could not update the reminder."); } });
    const t = head.createDiv();
    ...
    const btns = contentEl.createDiv({ cls: "orc-detail-btns" });
    btns.createEl("button", { text: r.completed ? "Mark as not done" : "Mark as done" }).addEventListener("click", async () => { this.close(); try { await this.store.toggleReminder(r); } catch { new Notice("Could not update the reminder."); } });
