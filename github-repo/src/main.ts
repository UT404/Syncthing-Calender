...
export default class SyncthingCalendarPlugin extends Plugin {
  ...
  async onload() {
    this.settings = sanitizeSettings((await this.loadData()) ?? (await this.legacySettings()) ?? {});
  ...
  private async legacySettings(): Promise<Partial<Settings> | null> {
    try {
      const vault = this.app.vault as unknown as { configDir?: string; adapter?: { exists(p: string): Promise<boolean>; read(p: string): Promise<string> } };
      const p = `${vault.configDir ?? ".obsidian"}/plugins/${LEGACY_ID}/data.json`;
  ...
  private async trash(f: TAbstractFile) {
    const fmgr = this.app.fileManager as unknown as { trashFile?: (f: TAbstractFile) => Promise<void> };
...
function sanitizeSettings(raw: Partial<Settings>): Settings {
  ...
  return {
    ...
    listColors: Object.fromEntries(Object.entries(s.listColors && typeof s.listColors === "object" ? s.listColors : {}).filter(([, v]) => typeof v === "string" && HEX.test(v))) as Record<string, string>,
