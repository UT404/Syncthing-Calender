...
export function parseReminder(fm: Record<string, any> | undefined, path: string, basename: string, list: Calendar): RawEvent | null {
  ...
  const priority: Priority = ["low", "medium", "high"].includes(fm.priority) ? fm.priority : "none";
  const repeat: ReminderRepeat = REMINDER_REPEATS.includes(fm.repeat) && due ? fm.repeat : "none";
